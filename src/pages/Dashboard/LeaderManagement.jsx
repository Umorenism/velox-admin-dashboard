...
export default function LeaderManagement() {
  ...
  // Fetch data from the stats/users endpoint
  useEffect(() => {
    const fetchStatsData = async () => {
      try {
        const res = await apiClient.get("/api/admin/stats/users");
        console.log("✅ Stats API raw response:", res.data);
        ...
        if (res.data && typeof res.data === 'object') {
          if ('totalLeaders' in res.data && 'totalActiveUsers' in res.data) {
            totalLeaders = res.data.totalLeaders;
            totalActiveUsers = res.data.totalActiveUsers;
          } else if (res.data.stats) {
            totalLeaders = res.data.stats.totalLeaders || 0;
            totalActiveUsers = res.data.stats.totalActiveUsers || 0;
          } else if (Array.isArray(res.data.result)) {
            totalLeaders = res.data.result.find(item => item.key === 'totalLeaders')?.value || 0;
            totalActiveUsers = res.data.result.find(item => item.key === 'totalActiveUsers')?.value || 0;
          } else {
        ...
        const transformedData = [
          { title: "Total Leaders", amount: totalLeaders.toString(), currency: "USD" },
          { title: "Total Active Users", amount: totalActiveUsers.toString(), currency: "USD" },
        ];

        setData(transformedData);
      } catch (err) {
        console.error("❌ Stats fetch error:", err);
        setError(err.response?.data?.message || "Failed to fetch stats data");
      } finally {
        setLoading(false);
      }
    };
        ...
        <div className="flex flex-col md:flex-row justify-center sm:justify-start gap-4">
          {loading ? (
            [0, 1].map((index) => (
              ...
              >
                <div className="flex items-center justify-between w-full mb-2">
                  <h3 className="text-[15px] font-semibold">{index === 0 ? "Total Leaders" : "Total Active Users"}</h3>
                  <div className="w-7 h-7 rounded-full bg-gray-100 dark:bg-neutral-800 flex justify-center items-center border border-gray-300 dark:border-neutral-700">
                    <DollarSign size={16} />
                  </div>
                </div>
                <div className="border-t border-neutral-200 dark:border-neutral-800 w-full mt-3 pt-2">
                  <span className="text-[28px] sm:text-[32px] font-bold text-gray-800 dark:text-gray-100">
                    Loading...
                  </span>
                </div>
              </motion.div>
            ...
            [0, 1].map((index) => (
              ...
              >
                ...
                <div className="border-t border-neutral-200 dark:border-neutral-800 w-full mt-3 pt-2">
                  <span className="text-[28px] sm:text-[32px] font-bold text-red-500 dark:text-red-400">
                    Error: {error}
                  </span>
                </div>
              </motion.div>
