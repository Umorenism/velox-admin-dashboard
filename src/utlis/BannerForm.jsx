...
import { createBanner } from "../api/bannerApi"; // ✅ import API
...
const BannerForm = ({ onBannerCreated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("urlLink", banner.link);
      formData.append("active", banner.status === "active");
      formData.append("startDate", banner.startDate);
      formData.append("endDate", banner.endDate);
      formData.append("image", banner.image);

      const res = await createBanner(formData);
      console.log("✅ Create Banner Response:", res);

      setMessage("✅ " + res.message);
      if (onBannerCreated) onBannerCreated(res.banner);
          ...
          {message && (
            <p
              className={`text-sm font-medium ${
                message.startsWith("✅") ? "text-teal-600" : "text-red-500"
              }`}
            >
              {message}
            </p>
